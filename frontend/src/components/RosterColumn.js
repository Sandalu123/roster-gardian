...
import { FiUser, FiPhone, FiMail, FiPlus, FiMessageSquare } from 'react-icons/fi';
...
const RosterColumn = ({ date, isToday, roster, issues, onSelectIssue, onCreateIssue, canCreateIssue }) => {
  ...
  return (
    <div className={`flex-shrink-0 w-80 border-r border-gray-200 ${isToday ? 'bg-blue-50' : 'bg-white'}`}>
      <div className={`sticky top-0 z-10 ${isToday ? 'bg-blue-100' : 'bg-gray-50'} border-b border-gray-200 p-4`}>
        <div className="text-center">
          ...
          {isToday && (
            <div className="mt-2 text-xs font-semibold text-blue-600">TODAY</div>
          )}
        </div>
      </div>
              ...
              <div key={person.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <span className={`inline-block text-xs px-2 py-1 rounded-full ${getRoleBadgeColor(person.role)}`}>
                      {person.role}
                    </span>
